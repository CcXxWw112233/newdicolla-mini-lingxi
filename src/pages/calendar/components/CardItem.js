...
export default class CardItem extends Component {
  ...
  render() {
    ...
    return (
      <View onClick={() => this.gotoListItemDetails(itemValue)}>
        <View className={`${globalStyles.global_card_out} ${indexStyles.card_content} `} style={`opacity: ${dis_due_style()}`}>
          ...
          <View className={`${indexStyles.card_content_middle}`}>
            ...
            <View className={`${indexStyles.card_content_middle_bott}`} style={{ color: (now > due_time && flag != '1' && is_realize != '1') ? '#F5222D' : '#8c8c8c' }}>
